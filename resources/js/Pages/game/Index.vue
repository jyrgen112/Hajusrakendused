    ...
    <div class="max-w-7xl mx-auto p-6">
      ...
      <div class="flex gap-3 mb-8">
        <button
          ...
          :class="activeTab === 'games' ? 'bg-indigo-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'"
          class="px-6 py-2 rounded-xl font-semibold transition"
        >
          🎮 My Games
        </button>
        <button
          @click="activeTab = 'movies'; loadMovies()"
          :class="activeTab === 'movies' ? 'bg-indigo-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'"
          class="px-6 py-2 rounded-xl font-semibold transition"
        >
          🎬 Movies API
        </button>
      ...
      <div v-if="activeTab === 'movies'">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-2xl font-bold">🎬 Movies — External API</h2>
          <p class="text-gray-400 text-sm">Source: ralfiharjutus.ta24siim.itmajakas.ee</p>
        </div>

        <!-- Search & Filter Movies -->
        <div class="flex flex-wrap gap-4 mb-6">
          <input v-model="movieSearch" type="text" placeholder="Search movies..." class="bg-gray-800 rounded-xl px-4 py-2 text-white placeholder-gray-400 outline-none flex-1 min-w-48" />
          <select v-model="movieGenre" class="bg-gray-800 rounded-xl px-4 py-2 text-white outline-none">
            <option value="">All Genres</option>
        ...
        <!-- Loading -->
        <div v-if="moviesLoading" class="text-center py-12 text-gray-400">Loading movies...</div>

        <!-- Error -->
        <div v-else-if="moviesError" class="text-center py-12 text-red-400">Failed to load movies. Try again.</div>

        <!-- Movies Grid -->
        <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <div v-for="movie in filteredMovies" :key="movie.id" class="bg-gray-800 rounded-2xl overflow-hidden hover:ring-2 hover:ring-purple-500 transition">
            <img v-if="movie.image" :src="movie.image" :alt="movie.title" class="w-full h-48 object-cover" />
            <div v-else class="w-full h-48 bg-gray-700 flex items-center justify-center text-5xl">🎬</div>
            <div class="p-5">
              <div class="flex justify-between items-start mb-2">
                <h2 class="text-lg font-bold">{{ movie.title }}</h2>
                <span class="text-xs bg-purple-600 px-2 py-1 rounded-full">{{ movie.genre }}</span>
              </div>
              <p class="text-gray-400 text-sm mb-1">{{ movie.release_year }} · Dir. {{ movie.director }}</p>
              <p class="text-yellow-400 text-sm mb-2">⭐ {{ movie.rating }}/10</p>
              <p class="text-gray-300 text-sm leading-relaxed">{{ movie.description }}</p>
            </div>
          </div>
          <p v-if="filteredMovies.length === 0" class="text-gray-400 col-span-3 text-center py-12">No movies found!</p>
        </div>
      </div>
...
const movieGenres = computed(() => [...new Set(movies.value.map(m => m.genre))].sort())

const filteredMovies = computed(() => {
  let result = [...movies.value]
  if (movieSearch.value) {
    result = result.filter(m => m.title.toLowerCase().includes(movieSearch.value.toLowerCase()))
  }
  if (movieGenre.value) {
    result = result.filter(m => m.genre === movieGenre.value)
  }
  return result
})
...
async function loadMovies() {
  ...
  try {
    const res = await fetch('https://ralfiharjutus.ta24siim.itmajakas.ee/api/movies')
